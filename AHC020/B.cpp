...
struct Field {
  ...
  vector<vector<int>> bedrocks;
  int total_cost;

  Field(int N, int C, vector<vector<int>> &bedrocks)
      : N(N), C(C), is_broken(N, vector<int>(N, 0)), bedrocks(bedrocks) {}
...
struct Solver {
  ...
  Solver(int N, vector<P> &sources, vector<P> &houses, int C,
         vector<vector<int>> &bedrocks)
      : N(N), sources(sources), houses(houses), C(C), field(N, C, bedrocks) {}
  void solve() {
    // 全ての家から0番目の水源へ繋げる
    for (P house : houses) {
      int pos = 0;
...
int main() {
  ...
  Solver solver(N, sources, houses, C, bedrocks);
  solver.solve();
}
